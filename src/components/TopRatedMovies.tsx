...
const TopRatedMovies: React.FC = () => {
  ...
  useEffect(() => {
    dispatch(
      fetchTopRatedMoviesThunk({ page: currentPage, genre: selectedGenre })
    );
  ...
  const handleGenreChange = (event: SelectChangeEvent<number>) => {
    ...
    dispatch(
      fetchTopRatedMoviesThunk({ page: 1, genre: value === '' ? null : value })
    );
  };
  ...
  const handlePageChange = (page: number) => {
    dispatch(setTopRatedCurrentPage(page));
    ...
    <Box sx={{ px: { xs: 2, md: 4 }, pb: 4, transition: 'all 0.3s' }}>
      ...
      >
        ...
        <FormControl sx={{ minWidth: 150 }} size="small">
          <InputLabel id="toprated-genre-select-label">Genre</InputLabel>
          <Select
            labelId="toprated-genre-select-label"
            value={selectedGenre || ''}
            label="Genre"
